...
def extract_test_scripts(project_name=None, testplan_name=None):
    ...
    print(f"Projet trouvé : {project_name} (ID: {project_id})")
    ...
    if not testplan:
        print(f"Plan de test '{testplan_name}' non trouvé.")
        return

    testplan_id = testplan['id']
    print(f"Plan de test trouvé : {testplan_name} (ID: {testplan_id})")

    # 3. Récupérer les cas de test associés
    # 3. Récupérer les cas de test associés
    testcases = tlc.getTestCasesForTestPlan(testplan_id)
    print(f"{len(testcases)} cas de test trouvés dans le plan '{testplan_name}':")

# 4. Extraire et afficher les scripts
    if isinstance(testcases, list):
       for tc in testcases:
        print("—" * 40)
        print(f"ID: {tc.get('tc_id', 'N/A')}")
